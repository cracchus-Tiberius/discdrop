// scripts/scrape-nydisk.js — standalone scraper for nydisk.no (Shopify)
// Attempt 1: Shopify public products.json API
// Attempt 2: Playwright headless Chromium
// Usage: node scripts/scrape-nydisk.js  or  pnpm scrape:nydisk
'use strict';
...
const STORE = {
  ...
  // No single published flat rate below the free-shipping threshold ("Prises
  // beregnes i kassa" — calculated at checkout) — 45 kr matches every other
  // Norwegian domestic store in this config.
...
async function scrapeWithApi() {
  ...
  while (true) {
    ...
    for (const product of products) {
      ...
      // Skip used/second-hand and mini marker products (name + product_type/tags)
...
async function main() {
  ...
  console.log(`NyDisk scraper — ${now}`);
