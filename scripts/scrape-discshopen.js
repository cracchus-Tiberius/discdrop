// scripts/scrape-discshopen.js — standalone scraper for discshopen.no (WooCommerce)
...
// Usage: node scripts/scrape-discshopen.js  or  pnpm scrape:discshopen
'use strict';
...
const STORE = {
  ...
  // "Vi tilbyr gratis frakt på alle bestillinger over kr. 699,-" — no flat
  // rate below that is published; 45 kr matches every other Norwegian
  // domestic store in this config.
  freeShippingOver: 699,
  shipping: 45,
  // No single "all discs" category (products are split across many type/brand
  // taxonomy terms) — the general shop page for the Playwright fallback,
  // relying on isNonDiscProduct() to filter out bags/accessories like every
  // other scraper does.
...
async function scrapeWithPlaywright() {
  ...
  try {
    ...
    for (const categoryUrl of STORE.playwrightCategoryUrls) {
...
async function main() {
  ...
  console.log(`DiscShopen scraper — ${now}`);
