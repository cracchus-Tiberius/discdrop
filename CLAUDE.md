...
- Discsport blocks GitHub Actions. Its LiteSpeed server answers runner IPs
  with a reCAPTCHA "Bot Verification" page — HTTP 200, 1705 bytes, every path,
  every user agent — while a residential connection gets the real site.
  Probed from a runner 2026-09-13. That is the cause of the 2026-08-18 outage
  and of the one from 2026-09-05 onward (stale 5+ days, failure e-mailed daily).
  A 2026-09-08 fix blamed Angular hydration and moved the slug step from
  Playwright to plain fetch; that theory was only ever tested from a
  workstation, which the block doesn't apply to, and it changed nothing. The
  lesson: a scraper that works locally and fails in CI must be probed FROM
  CI before diagnosing — the runner and the laptop are not seeing the same
  site. The scraper now detects the captcha page and logs BLOCKED. Do not
  build around the captcha: it is the store declining datacenter bots.
